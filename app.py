...
#
@app.route("/downloadzipfile", methods=["POST", "GET"])
def downloadzipfile():
    majors =  Majors.query.all()
    levels =  Levels.query.all()
    return render_template("download_barcode.html", majors=majors, levels=levels)

@app.route("/zipfile", methods=["POST", "GET"])
def gzipfile():
    if request.method == "POST":
        major = request.form.get("major")
        level = request.form.get("level")
        if major and level:
            ...
            with zipfile.ZipFile(memory_file, 'w') as zipObj2:
                # Add multiple files to the zip
                students = Students.query.filter_by(major_id = major, level_id = level).all()
                os.chdir('barcode_path')
                major_name = ""
                level_name = ""
                for st in students:
                    zipObj2.write(st.image_name)
                    major_name = st.major.name
                    level_name = st.level.name
                zipObj2.close()
                os.chdir("../")
            memory_file.seek(0)
            return send_file(memory_file, mimetype='application/x-7z-compressed', attachment_filename="{}-{}.zip".format(major_name,level_name), as_attachment=True)
...
# def print_image():
#     print(os.startfile("icon.png", "print"))
